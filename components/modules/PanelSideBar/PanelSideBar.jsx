...
let links = [
    { title: 'Dashboard', href: '/p-user/', icon: <LuLayoutDashboard className="text-white text-xl" /> },
    { title: 'Profile Edit', href: '/p-user/profile-edit', icon: <IoSettingsOutline className="text-white text-xl" /> },
    {
        title: 'Shopping Cart', href: '/p-user/cart', icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-white w-5 h-5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
        </svg>
    },
    { title: 'Orders History', href: '/p-user/orders', icon: <IoIosAddCircleOutline className="text-white text-xl" /> },
    { title: 'Booking History', href: '/p-user/booking', icon: <BsCalendarDate className="text-white text-xl" /> },
    { title: 'Comments History', href: '/p-user/comments', icon: <AiOutlineComment className="text-white text-xl" /> },
]
            ...
            <div className={`w-full z-50 fixed left-0 top-0 ${showMenu ? 'translate-x-0' : 'translate-x-full'} lg:translate-x-0 transition-all duration-300 bg-black shadow shadow-black/5 dark:bg-secondary lg:w-1/4 h-screen `}>
                <div className="px-5 py-9 flex flex-col justify-start items-center gap-5 overflow-y-scroll lg:overflow-hidden">
                    ...
                    <div className="w-full flex items-center justify-center gap-4">
                        <Link href='/p-user/cart' className="relative flex items-center p-2 xl:p-3 rounded-xl border border-[#1f1f1f] bg-[#0f0f0f] cursor-pointer transition-all group">
                            ...
                            <span className="inline-block opacity-0 h-5 absolute left-1/2 -top-5 -translate-1/2  border border-[#1f1f1f] bg-[#0f0f0f] px-2 py-0.5 rounded-md text-xs text-nowrap z-20 group-hover:opacity-100 transition-all">Shopping Cart</span>
                            <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-sky-500 flex items-center justify-center text-sm text-white">{user?.cart?.items.length || 0}</span>
                        </Link>
