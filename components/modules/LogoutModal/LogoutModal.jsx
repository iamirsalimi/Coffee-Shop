...
export default function LogoutModal({ showModal, setShowModal, isLoggingOut, logoutHandler }) {
    const hideModal = () => {
        setShowModal(false)
    }
    ...
    return (
        <div className={`absolute w-full top-0 left-0 h-full z-50 flex items-center justify-center transition-all ${showModal ? 'visible' : 'invisible'}`}>
            ...
            <div className={`w-[90%] sm:w-4/5 lg:w-1/3 fixed my-auto -mt-20 h-fit border border-[#1f1f1f] bg-[#0f0f0f] py-7 px-5 rounded-xl dark:bg-primary flex flex-col items-center gap-7 transition-all duration-200 ${showModal ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
                <h2 className="text-light-gray dark:text-white font-bold text-lg lg:text-xl text-center">Are you sure you want to log out your account ?</h2>
                ...
                <div className="w-full grid grid-cols-2 gap-2">
                    ...
                    >{isLoggingOut ? 'Logging out' : 'yep'}</button>
                </div>
            </div>
        </div>
    )
