    ...
    return (
        <div className="pb-16 container px-5 py-1 mx-auto relative w-full min-h-screen flex flex-col md:flex-row-reverse items-center justify-start gap-4">
            ...
            <div className="w-full md:w-1/2 min-h-4/5 xs:h-full md:h-[calc(100vh-1rem)] rounded-2xl overflow-hidden">
                <img src="Images/About-1.jpg" className="object-cover object-center w-full h-full" alt="" />
            </div>
            {/* left-side header */}
            <div className="md:pr-5 w-full md:w-1/2 rounded-md flex flex-col md:items-start items-center justify-center gap-4 sm:gap-6 ">
                <h1 className=" text-white font-bold text-3xl sm:text-4xl md:text-justify text-center">More Than Just Coffee</h1>
                <p className="md:text-justify text-center md:text-lg text-gray-400">Coffee is more than a daily habit — it’s a moment of pause in a busy day. At our café, every detail matters, from the beans we choose to the space we create. We believe in quality, comfort, and connection. Every cup is crafted to be enjoyed, not rushed</p>
