...
function AddProduct({ user, product }) {
    ...
    const changeValues = product => {
        ...
        setValue('smallPrice', product?.smallPrice, { shouldValidate: true })
    ...
    const editProductHandler = async (data) => {
        ...
        newProduct.append('title', data.title || product.title)
        newProduct.append('slug', data.slug.split(' ').join('-') || product.slug)
        newProduct.append('summary', data.summary || product.summary)
        newProduct.append('description', data.description || product.description)
        newProduct.append('smallPrice', data.smallPrice || product.smallPrice)
    ...
    return (
        <div className="w-full lg:w-3/4 min-h-screen pb-20 md:pb-10 lg:pb-0 flex flex-col gap-7 bg-[#0f0f0f] ml-auto p-4 pb-32">
            ...
            <form className="min-h-screen h-full mb-16 grid grid-cols-1 md:grid-cols-2 gap-y-9 gap-x-3 border border-[#1f1f1f] bg-black px-5 pt-12 pb-4 rounded-3xl" onSubmit={handleSubmit(editProductHandler)}>
                {/* title */}
                ...
                <div className="md:col-start-1 md:col-end-3 w-full relative select-none">
                    ...
                    <label htmlFor="productImageFile" className="w-full min-h-52 flex flex-col items-center justify-center gap-2 cursor-pointer rounded-3xl border-2 border-dashed border-[#1f1f1f] bg-[#0f0f0f]">
                        <div className="flex flex-col items-center justify-center gap-1">
                            ...
                            <span className="text-gray-500 text-sm text-center">this product already have an image , for checking it you can visit <Link href={`/p-admin/products/${product?.slug}`} className="underline">product details</Link> page , if you want to change that , just add your image and update product , it will remove the other picture automatically</span>
                        </div>
                        {file && (
